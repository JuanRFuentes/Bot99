...
╭─❒ 「 🐱 Bot Info 🐱 」 
├ 👤 Creator ➡️  @${ownernya.split('@')[0]}
├ 😎 Powered  ➡️ @${ini_mark.split('@')[0]}
├ 🍏 Prefix ➡️   ${prefix}
├ 📋 Total hit ➡️ ${hitall}
├ ⚔️ Hit today ➡️ ${hit_today.length}
├ 🏃 ‍Speed ➡️ ${latensii.toFixed(4)} Second
├ 🔥 Memory Used ➡️ ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB
├ ☕ Hostname ➡️ ${os.hostname()}
├ 🍵 Platform ➡️ ${os.platform()}
├ 🍯 Private chat ➡️ ${(Object.keys(db.data.chats).filter(v => v.endsWith('@s.whatsapp.net')).map(v => v).length)}
├ 📦 Group chat ➡️ ${(Object.keys(db.data.chats).filter(v => v.endsWith('@g.us')).map(v => v).length)}
├ 📝 Total chats ➡️  ${(Object.keys(db.data.chats).filter(v => v.endsWith('@s.whatsapp.net')).map(v => v).length) + (Object.keys(db.data.chats).filter(v => v.endsWith('@g.us')).map(v => v).length)} 
├ ⚙️ User In Database ➡️ ${Object.keys(global.db.data.users).length} Users
├ ⚖️ User Registered ➡️ ${(Object.values(global.db.data.users).filter(user => user.registered == true).length)}
╰❒ 🧽 Runtime 🧽 ➡️ ${runtime(process.uptime())}

╭─❒ 「 🧻 Date Info 🧻 」 
├ 🙂 Masehi ➡️ ${week}, ${date}
├ 😊 Hijriah ➡️ ${dateIslamic}
╰❒

╭─❒ 「 🐺 User Info 🐺 」 
├ 🦁 Register➡️ ${user.registered ? `✅\n├ Name : ${user.name}\n├ Age : ${user.age} years\n├ Gender : ${user.gender}\n├ Hobby : ${user.hobi}\n├ Registered on : ${tanggal(user.regTime)}` : '❌'}
├ 🐪 Bio ➡️ ${bio}
├ 🐱 Nomor ➡️ @${m.sender.split('@')[0]}
├ 🐥 Limit ➡️ ${limit} limit
├ 🐼 XP ➡️ ${exp}
├ 🐹 Premium ➡️ ${isPremium ? '✅' : '❌'}
├ 🐭 Me ➡️ ${m.key.fromMe ? 'True' : 'False'}
╰❒ 🐈 Owner 🐈 ➡️ ${isCreator ? 'True' : `False`}
`	
const buttojns = [
  {buttonId: 'command', buttonText: {displayText: '🌀 LIST MENU 🌀'}, type: 1},
  {buttonId: 'owner', buttonText: {displayText: '⭐ OWNER ⭐'}, type: 1},
  {buttonId: 'donasi', buttonText: {displayText: '☃️ DONASI ☃️'}, type: 1}
]			
